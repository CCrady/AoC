...
positionMap :: Int -> Galaxies -> Point -> Point
positionMap growth galaxies@((width, height), _) (x, y) = (colMap ! x, rowMap ! y) where
    ...
    coordMap empty size = coordMap' 1 1 empties where
        ...
        coordMap' iIn iOut (e:es)
            ...
            | otherwise  = iOut : coordMap' (iIn+1) (iOut+growth) (es)

expand :: Int -> Galaxies -> [Point]
expand growth galaxies@(_, points) = map (positionMap growth galaxies) points
...
solve growth = sum . map (uncurry dist) . pairs . (expand growth) . parse

main = puzzleInput "11" $ solve 1000000
test = puzzleInputTest "11" $ putStrLn . show . solve 10
