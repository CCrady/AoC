...
findEnergized :: Contraption -> Heading -> Beams
findEnergized contraption startHeading = propagate startHeading startBeams where
...
startHeadings :: Index -> [Heading]
startHeadings (width, height) = [ ((1, y),      East)  | y <- [1..height] ]
                             ++ [ ((x, 1),      South) | x <- [1..width]  ]
                             ++ [ ((width, y),  West)  | y <- [1..height] ]
                             ++ [ ((x, height), North) | x <- [1..width]  ]


solve str = let
    contraption = parseContraption str
    (_, size) = bounds contraption
    in maximum $ map (countEnergized . findEnergized contraption) $ startHeadings size
main = puzzleInput "16" solve
test = puzzleInputTest "16" $ putStrLn . show . solve
