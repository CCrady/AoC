...
indicesButOneEqual :: Grid -> [Index] -> [Index] -> Bool
indicesButOneEqual grid is js = let
    equalities = zipWith (\i j -> grid ! i == grid ! j) is js
    dropped = dropWhile id equalities
    (False : afterFalse) = dropped
    -- dropped is [] iff there are no Falses
    -- afterFalse contains a False iff there is more than one False
    in not (null dropped) && and afterFalse
...
isRowMirrored grid rowNum = let
    ...
    in indicesButOneEqual grid aboveIndices belowIndices
...
isColMirrored grid colNum = let
    ...
    in indicesButOneEqual grid leftIndices rightIndices
