...
reachableFrom lm@(_, Momentum _ inertia) bnds = let
    turnLms = [turn turnCCW lm, turn turnCW lm]
    straightLm = goStraight lm
    newLms = if inertia < 4
        then [straightLm]
        else straightLm : turnLms
...
lossArrayBounds atlas = let
    ...
    in ((lo, Momentum North 1), (hi, Momentum West 10))
...
getStartState atlas bnds = do
    let northStart = ((1, 1), Momentum North 4)
    let westStart  = ((1, 1), Momentum West  4)
